...
const storesIds = [new mongoose.Types.ObjectId(), new mongoose.Types.ObjectId()];
...
async function createHashedPassowrd(user_raw) {
    const user_raw = faker.internet.password();
    const salt = await bcrypt.genSalt();
    const hash = await bcrypt.hash(user_raw, salt);
    passwordToHashMap.push({ passwor: user_raw, hash: hash });
    return hash;
}
// Generate fake user
const fakeUser = {
